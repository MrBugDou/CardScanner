...
// https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardBinCheck=truecardNo=?
// {"cardType": "DC","bank": "CCB","key": "6217000010067674070","messages":[],"validated":true,"stat": "ok"}
...
/// bank银行代码及银行中文名对应JSON
private let bankCodeJson = [
    "SRCB": "深圳农村商业银行",
    "BGB": "广西北部湾银行",
    "SHRCB": "上海农村商业银行",
    "BJBANK": "北京银行",
...
/// cardType银行卡类别码对应中文名JSON
private let cardTypeJson = [
    "DC": "储蓄卡",
    "CC": "信用卡",
    "SCC": "准贷记卡",
    "PC": "预付费卡"
...
public struct BankCard {
    ...
    /// 卡类型名称
    public private(set) var cardTypeName: String?
    /// 是否验证成功
    public private(set) var validated: Bool = false
    /// 银行LOGO
    public private(set) var bankImg: String?
    
    public init(number: String) {
        self.number = number
        self.bankName = BankCard.getBankName(by: number)
    }
    
    public static func getBankName(by number: String) -> String? {
        var bankName: String?
        let bankNumber = number.replacingOccurrences(of: " ", with: "")
        for idx in stride(from: 0, to: strBankBinArray.count, by: 2) {
            let code = strBankBinArray[idx]
            if bankNumber.count < code.count {
                continue
            }
            if bankNumber.hasPrefix(code) {
                let numberLength = bankBinArrayCount[idx/2]
                if numberLength == 0 || numberLength == bankNumber.count {
                    bankName = strBankBinArray[idx+1]
                    break
                }
            }
        }
